...
export default function ThreadedComments({ postId, currentUserId, onCountChange }: Props) {
  ...
  async function loadReplies(parent: ThreadedComment, append = false) {
    ...
    const { data, error: queryError } = await query
    if (queryError) setError('Não foi possível carregar as respostas.')
    else {
  ...
  if (loading) return <div role="status" className="flex items-center gap-2 py-4 text-sm text-zinc-500"><Loader2 className="h-4 w-4 animate-spin motion-reduce:animate-none" /> Carregando comentários</div>

  return <section aria-label="Comentários" className="mt-4 border-t border-zinc-200/70 pt-4 dark:border-zinc-800/70">
    <div className="mb-3 flex items-center justify-between gap-3">
      <h3 className="text-sm font-black text-zinc-800 dark:text-zinc-100">Conversa</h3>
      <span className="text-xs font-semibold text-zinc-500">10 por vez</span>
    </div>
    ...
    {roots.length === 0 ? <p className="py-3 text-sm text-zinc-500">Seja a primeira pessoa a comentar.</p> :
    ...
    {hasMoreRoots && <button type="button" onClick={() => void loadRoots(true)} disabled={loadingTarget === 'roots'} className="mt-4 min-h-11 w-full rounded-full border border-zinc-300 px-4 text-sm font-bold dark:border-zinc-700">
      {loadingTarget === 'roots' ? 'Carregando…' : 'Carregar mais comentários'}
    </button>}
  </section>
}
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            ...
            {!comment.deleted_at && <div className="relative">
              <button type="button" onClick={() => setMenu(!menu)} aria-label="Opções do comentário" className="min-h-10 min-w-10 rounded-full hover:bg-zinc-200 dark:hover:bg-zinc-800"><MoreHorizontal className="mx-auto h-4 w-4" /></button>
              {menu && <div className="absolute right-0 z-20 w-36 rounded-xl border bg-white p-1 shadow-xl dark:border-zinc-700 dark:bg-zinc-950">
                {comment.user_id === currentUserId ? <>
                  <button type="button" onClick={() => { setEditing(true); setMenu(false) }} className="min-h-10 w-full rounded-lg px-3 text-left text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800">Editar</button>
                  <button type="button" onClick={async () => { setMenu(false); if (!window.confirm('Excluir este comentário?')) return; const { error } = await supabase.rpc('delete_threaded_comment', { p_comment_id: comment.id }); if (!error) await onRefresh(comment) }} className="min-h-10 w-full rounded-lg px-3 text-left text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30">Excluir</button>
                </> : <button type="button" disabled={reported} onClick={async () => {
                  const reason = window.prompt('Informe o motivo da denúncia (10 a 500 caracteres).')
                  if (!reason) return
                ...
                }} className="min-h-10 w-full rounded-lg px-3 text-left text-sm text-red-600 hover:bg-red-50 disabled:text-zinc-400 dark:hover:bg-red-950/30">{reported ? 'Denunciado' : 'Denunciar'}</button>}
              </div>}
            </div>}
          </div>
          {comment.deleted_at ? <p className="mt-2 italic text-zinc-500" aria-label="Comentário removido">Comentário removido</p> :
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-zinc-500">
            {!comment.deleted_at && <button ref={trigger} type="button" onClick={() => setReplying(true)} className="min-h-9 rounded-full px-2 font-bold hover:bg-zinc-200 dark:hover:bg-zinc-800">Responder</button>}
...
function CommentComposer({ mode, comment, currentUserId, inputRef, onCancel, onSaved }: { mode: 'reply' | 'edit'; comment: ThreadedComment; currentUserId: string; inputRef: React.RefObject<HTMLTextAreaElement | null>; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit() {
    if (!commentHasContent(content, expression)) { setError('Escreva uma resposta ou escolha uma expressão.'); return }
  ...
  return <div className="mt-3 rounded-xl border border-zinc-200 bg-white p-2 dark:border-zinc-700 dark:bg-zinc-950">
    <label className="sr-only" htmlFor={`composer-${mode}-${comment.id}`}>{mode === 'reply' ? `Responder a ${comment.profiles?.username || 'comentário'}` : 'Editar comentário'}</label>
    <textarea ref={inputRef} id={`composer-${mode}-${comment.id}`} value={content} onChange={(event) => setContent(event.target.value.slice(0, 2000))} placeholder={mode === 'reply' ? `Responder a @${comment.profiles?.username || 'usuario'}` : 'Editar comentário'} className="min-h-20 w-full resize-none bg-transparent p-2 text-sm outline-none" />
    {expression && <div className="flex items-start gap-2 p-2"><ExpressionAttachment expression={expression} compact /><button type="button" aria-label="Remover expressão" onClick={() => setExpression(null)}><X className="h-4 w-4" /></button></div>}
    {error && <p role="alert" className="px-2 text-xs text-red-600">{error}</p>}
    <div className="flex items-center justify-between gap-2">
      <button type="button" onClick={() => setPicker(true)} className="min-h-10 rounded-full px-3 text-xs font-bold hover:bg-zinc-100 dark:hover:bg-zinc-800">Emoji, GIF ou sticker</button>
      <div className="flex gap-1"><button type="button" onClick={onCancel} className="min-h-10 rounded-full px-3 text-xs font-bold">Cancelar</button><button type="button" onClick={() => void submit()} disabled={sending} className="flex min-h-10 items-center gap-1 rounded-full bg-blue-600 px-3 text-xs font-bold text-white disabled:opacity-60">{sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}{mode === 'reply' ? 'Responder' : 'Salvar'}</button></div>
    </div>
