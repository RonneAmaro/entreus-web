  ...
  return (
    <>
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[76px] border-r border-zinc-200 bg-white/95 backdrop-blur-xl dark:border-blue-400/10 dark:bg-black/95 lg:block">
        ...
        <nav aria-label="Navegação principal" className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-2 xl:gap-3">
          <Link href="/feed" className={linkClass('/feed')} aria-label="Casa" aria-current={isNavigationItemActive(pathname, '/feed') ? 'page' : undefined} title="Casa"><Home className="h-5 w-5" /></Link>
          <Link href="/messages" className={linkClass('/messages')} aria-label="Mensagens" aria-current={isNavigationItemActive(pathname, '/messages') ? 'page' : undefined} title="Mensagens"><MessageCircle className="h-5 w-5" />{badge(runtime.unreadMessages)}</Link>
          <button ref={hubButtonRef} type="button" onClick={() => setHubOpen(true)} aria-label="Abrir Hub EntreUS" aria-expanded={hubOpen} aria-controls="entreus-hub" title="EntreUS" className="relative flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-400 to-blue-700 text-white shadow-lg shadow-blue-500/25 transition hover:scale-[1.04] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300 motion-reduce:transform-none"><Image src="/logo-icon.png" alt="" width={38} height={38} className="h-9 w-9 rounded-full object-contain" /></button>
          <Link href="/profile" className={linkClass('/profile')} aria-label="Perfil" aria-current={isNavigationItemActive(pathname, '/profile') ? 'page' : undefined} title="Perfil">{avatarUrl ? (
          ...
          <button type="button" onClick={post} aria-label="Postar" title="Postar" className="flex h-12 w-12 items-center justify-center rounded-2xl text-zinc-500 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400"><PenLine className="h-5 w-5" /></button>
        </nav>

        <Link href="/profile" aria-label={displayName ? `Abrir perfil de ${displayName}` : 'Abrir meu perfil'} title={displayName || username || 'Meu perfil'} className="absolute bottom-[max(env(safe-area-inset-bottom),16px)] left-1/2 flex h-11 w-11 -translate-x-1/2 items-center justify-center rounded-full text-zinc-400 ring-1 ring-white/10 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400">{avatarUrl ? (
          // eslint-disable-next-line @next/next/no-img-element -- Profile media can use approved runtime hosts outside next/image config.
