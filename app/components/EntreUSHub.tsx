...
import {
  ...
  PenLine, Settings, ShieldCheck, Sun, Trophy, User, Video, WalletCards, X,
} from 'lucide-react'
import {
  HUB_ITEMS,
...
const ICONS: Record<NavigationIcon, typeof Home> = {
  ...
  post: PenLine, search: Compass, settings: Settings, vip: Crown, wallet: WalletCards,
}
...
function navigationTitle(item: NavigationItem) {
  if (!item.title.startsWith('EntreUS')) return item.title
  return <><EntreUSWordmark />{item.title.slice('EntreUS'.length)}</>
}
...
export default function EntreUSHub(props: EntreUSHubProps) {
  ...
  function handleRouteClick(event: ReactMouseEvent<HTMLAnchorElement>, item: NavigationItem) {
    ...
    if (!isNavigationRouteItem(item) || pathname !== item.href) return
  ...
  function runItemAction(item: NavigationItem) {
    if (item.kind !== 'compose') return
    registerItemUsage(item)
    const composeHref = getComposeHref('text')
    if (pathname === '/feed') {
  ...
  const appLink = (item: NavigationItem, compact = false) => {
    ...
    const available = isNavigationItemAvailable(item)
    const className = `group relative flex min-w-0 rounded-2xl text-left transition duration-200 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 motion-reduce:transform-none motion-reduce:transition-colors ${available ? '' : 'cursor-not-allowed opacity-55'} ${compact ? 'items-center gap-3 px-2 py-2 hover:bg-zinc-100 dark:hover:bg-white/[0.07]' : 'min-h-24 flex-col items-center justify-start gap-2 px-2 py-3 text-center hover:-translate-y-0.5 hover:bg-zinc-100 dark:hover:bg-white/[0.07] sm:min-h-28 sm:px-3 sm:py-4'}`
    const content = (
      <>
