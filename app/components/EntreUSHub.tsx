...
const ACCENT_CLASSES: Record<NavigationAccent, string> = {
  amber: 'bg-amber-500/15 text-amber-200 ring-amber-300/20 group-hover:bg-amber-500/25 group-hover:shadow-amber-500/20',
  blue: 'bg-blue-500/15 text-blue-200 ring-blue-300/20 group-hover:bg-blue-500/25 group-hover:shadow-blue-500/20',
  cyan: 'bg-cyan-500/15 text-cyan-200 ring-cyan-300/20 group-hover:bg-cyan-500/25 group-hover:shadow-cyan-500/20',
  emerald: 'bg-emerald-500/15 text-emerald-200 ring-emerald-300/20 group-hover:bg-emerald-500/25 group-hover:shadow-emerald-500/20',
  fuchsia: 'bg-fuchsia-500/15 text-fuchsia-200 ring-fuchsia-300/20 group-hover:bg-fuchsia-500/25 group-hover:shadow-fuchsia-500/20',
  indigo: 'bg-indigo-500/15 text-indigo-200 ring-indigo-300/20 group-hover:bg-indigo-500/25 group-hover:shadow-indigo-500/20',
  red: 'bg-red-500/15 text-red-200 ring-red-300/20 group-hover:bg-red-500/25 group-hover:shadow-red-500/20',
  violet: 'bg-violet-500/15 text-violet-200 ring-violet-300/20 group-hover:bg-violet-500/25 group-hover:shadow-violet-500/20',
}
...
export default function EntreUSHub(props: EntreUSHubProps) {
  ...
  const appLink = (item: NavigationItem, compact = false) => {
    ...
    return (
      <Link key={item.id} href={item.href} onClick={() => openItem(item)} className={`group relative flex min-w-0 rounded-2xl text-left transition duration-200 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 motion-reduce:transform-none motion-reduce:transition-colors ${compact ? 'items-center gap-3 px-2 py-2 hover:bg-white/[0.07]' : 'min-h-24 flex-col items-center justify-start gap-2 px-2 py-3 text-center hover:-translate-y-0.5 hover:bg-white/[0.07] sm:min-h-28 sm:px-3 sm:py-4'}`}>
        ...
        <span className={`min-w-0 ${compact ? '' : 'flex w-full flex-col items-center text-center'}`}><span className={`block max-w-full font-bold text-white ${compact ? 'truncate text-sm' : 'min-h-8 text-xs leading-4 sm:text-sm'}`}>{item.title}</span>{!compact && <span className="mt-0.5 hidden max-w-44 text-center text-[11px] leading-4 text-zinc-500 sm:line-clamp-3">{item.description}</span>}</span>
      </Link>
    )
  }
  ...
  const content = (
    <div data-testid="entreus-hub-overlay" className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/75 p-0 backdrop-blur-md sm:p-5 lg:p-8" onMouseDown={(event) => { if (event.target === event.currentTarget) closeHub() }}>
      <div ref={dialogRef} id="entreus-hub" role="dialog" aria-modal="true" aria-labelledby="entreus-hub-title" className="relative flex h-[100dvh] w-full flex-col overflow-hidden bg-zinc-950 text-white shadow-2xl shadow-black/70 ring-1 ring-white/10 sm:h-[min(90dvh,820px)] sm:w-[min(88vw,1280px)] sm:rounded-[2rem]">
        ...
        <header className="relative shrink-0 px-4 pb-3 pt-[max(env(safe-area-inset-top),16px)] sm:px-8 sm:pb-5 sm:pt-7 lg:px-10">
          <div className="mb-4 flex items-center justify-between gap-4">
            ...
            <button type="button" onClick={closeHub} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-zinc-300 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400" aria-label="Fechar EntreUS"><X className="h-5 w-5" /></button>
          </div>
          <label className="relative mx-auto block w-full max-w-4xl"><Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-blue-300" /><span className="sr-only">Buscar no Hub</span><input ref={searchRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar aplicativos, páginas e recursos" className="h-[52px] w-full rounded-2xl border border-white/10 bg-white/[0.07] pl-12 pr-4 text-sm text-white shadow-inner outline-none transition placeholder:text-zinc-500 focus:border-blue-400/60 focus:bg-white/[0.09] focus:ring-4 focus:ring-blue-500/15 sm:h-14 sm:text-base" /></label>
        </header>
        ...
        <main className="relative flex-1 overflow-y-auto overscroll-contain px-4 pb-6 sm:px-8 lg:px-10">
          {query ? (
            ...
            <div className="mx-auto max-w-6xl py-4 sm:py-6">
              <section>
                <div className="mb-2 flex items-center justify-between"><h3 className="text-base font-black sm:text-lg">Fixados</h3><button type="button" onClick={() => setShowAll(true)} className="inline-flex min-h-11 items-center gap-1 rounded-xl px-3 text-sm font-bold text-blue-300 transition hover:bg-blue-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400">Ver todos <ChevronRight className="h-4 w-4" /></button></div>
        ...
        <footer className="relative flex shrink-0 items-center justify-between gap-3 border-t border-white/[0.07] bg-black/25 px-4 pb-[max(env(safe-area-inset-bottom),12px)] pt-3 backdrop-blur-xl sm:px-8 sm:py-3 lg:px-10">
          ...
          ) : <span className="flex h-9 w-9 items-center justify-center rounded-full bg-blue-500/15 text-blue-200"><User className="h-4 w-4" /></span>}<span className="min-w-0"><span className="block truncate text-sm font-bold">{props.displayName || props.username || 'Meu perfil'}</span><span className="hidden text-xs text-zinc-500 sm:block">Ver perfil</span></span></Link>
          <div className="flex items-center gap-1">{props.mounted && <button type="button" onClick={props.onToggleTheme} className="flex h-10 items-center gap-2 rounded-xl px-3 text-sm font-bold text-zinc-300 transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400" aria-label="Alternar tema">{props.theme === 'dark' ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}<span className="hidden sm:inline">Tema</span></button>}<button type="button" onClick={() => { closeHub(); props.onLogout() }} className="flex h-10 items-center gap-2 rounded-xl px-3 text-sm font-bold text-red-300 transition hover:bg-red-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400"><LogOut className="h-4 w-4" /><span className="hidden sm:inline">Sair</span></button></div>
        </footer>
      </div>
    </div>
  )
