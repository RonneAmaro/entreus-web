...
export default function MobileNavigation({
  ...
  return (
    <>
      <nav aria-label="Navegação principal" className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-5 items-end border-t border-white/10 bg-black/95 px-1 pb-[max(env(safe-area-inset-bottom),4px)] pt-1 backdrop-blur-xl lg:hidden">
        ...
        ) : <User className="h-5 w-5" />}<span>Perfil</span></Link>
        <button type="button" onClick={() => onPostClick('text')} className={itemClass(false)} aria-label="Postar"><PenLine className="h-5 w-5" /><span>Postar</span></button>
      </nav>
