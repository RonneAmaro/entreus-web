...
export default function AdminCreatorWithdrawalsPage() {
  ...
  async function updateWithdrawal(withdrawal: CreatorWithdrawal, action: 'paid' | 'rejected') {
    ...
    <main className="min-h-screen bg-black px-4 py-8 text-white">
      <section className="mx-auto w-full max-w-7xl">
        <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400">
              Pagamento Pix deve ser feito manualmente fora da plataforma. Esta tela apenas registra pago ou recusa.
            </p>
          </div>
        ...
        <div className="mb-5 rounded-3xl border border-amber-300/25 bg-amber-500/10 p-4 text-sm font-semibold leading-6 text-amber-50">
          Nenhum Pix automatico e executado aqui. Confira a chave Pix, pague fora da plataforma e so depois marque como pago.
        </div>
          ...
          {visibleWithdrawals.length === 0 ? (
            ...
            visibleWithdrawals.map((withdrawal) => {
              ...
              return (
                ...
                >
                  <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_repeat(6,minmax(0,1fr))_auto] xl:items-center">
                    ...
                    <div>
                      <p className="text-xs font-bold text-zinc-500">Tipo Pix</p>
                      <p className="text-sm font-semibold">{PIX_KEY_TYPE_LABELS[withdrawal.pix_key_type] || withdrawal.pix_key_type}</p>
                    </div>

                    <div>
                      <p className="text-xs font-bold text-zinc-500">Chave Pix</p>
                      <p className="break-all text-sm font-semibold">{withdrawal.pix_key}</p>
                    </div>

                    <div>
                      <p className="text-xs font-bold text-zinc-500">Titular</p>
                      <p className="text-sm font-semibold">{withdrawal.holder_name}</p>
                    </div>
                  ...
                  <div className="mt-4 grid gap-3 border-t border-white/10 pt-4 text-sm text-zinc-300 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] md:items-end">
                    <div>
                    ...
                    <label className="block">
                      ...
                      <textarea
                        ...
                        rows={3}
                        disabled={!isPending || actionLoadingId === withdrawal.id}
