...
export async function POST(request: Request) {
  try {
    ...
    if (typeof body?.requestId !== 'string') {
      return NextResponse.json({ ok: false, error: 'INVALID_PROOF_REQUEST' }, { status: 400 })
    }

    const { data: purchaseRequest, error: requestError } = await admin.supabase
    ...
    if (requestError) {
      return NextResponse.json({ ok: false, error: 'PROOF_LOOKUP_FAILED' }, { status: 500 })
    }
    ...
    const proof = purchaseRequest as PaymentProofRow
    const reference = proof.proof_path || proof.proof_url

    if (!reference) {
      return NextResponse.json({ ok: false, error: 'PROOF_NOT_AVAILABLE' }, { status: 404 })
    }

    const r2Key = getR2PrivateKey(reference)
    ...
    const storagePath = getSupabaseStoragePath(reference)
