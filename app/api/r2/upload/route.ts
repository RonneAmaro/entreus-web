...
function getFolder(formData: FormData) {
  ...
  return ACCEPTED_FOLDERS.has(normalizedFolder) ? normalizedFolder : null
}
...
export async function POST(request: Request) {
    ...
    return NextResponse.json(
      {
        ...
        message: 'Envie a requisicao como FormData.',
      },
      { status: 400 },
    )
  }
  ...
  const folder = getFolder(formData)
    ...
    return NextResponse.json(
      {
        ...
        message:
          'Folder invalido. Use apenas posts, avatars, banners ou messages.',
      },
      { status: 400 },
    )
  }
  ...
  const key = `${folder}/${Date.now()}-${crypto.randomUUID()}.${getExtension(file)}`
