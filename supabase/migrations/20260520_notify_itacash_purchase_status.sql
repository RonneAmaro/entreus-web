-- Notify users when manual ItaCash purchase requests are approved or rejected.
-- Review and apply manually in Supabase. Do not run automatically from Codex.

alter table public.notifications
  add column if not exists itacash_purchase_request_id uuid references public.itacash_purchase_requests(id) on delete set null;
